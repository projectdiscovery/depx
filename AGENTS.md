...
internal/sync/    Manifest-tracked index sync (OSV delta + PD)
internal/bundle/    Embedded offline bundles (pack/extract/seed)
cmd/embedintel/     Build-time tool: pack OSV/PD intel for //go:embed (CI + local)
...
- **`depx github -n N`**: caps repos when target is org/user (`config.NormalizeGitHubRepoLimit`)

Both use shared `config.normalizeLimit` — extend that helper rather than duplicating validation.

## Output cards

Malicious advisory cards share a header via **`output.writeMaliciousHeader`**. Feed and audit finding cards should reuse card helpers in `internal/output/card.go` instead of copying format strings.
...
3. **No `depx check` subcommand** — checks are `depx <ref>` or stdin.
...
| `DEPX_DISABLE_EMBEDDED_SEED` | Skip embedded offline bundle extraction (tests) |

When PD is enabled, **all** intel operations (feed, check, audit, id) use the PD API. The PD path lives under `internal/pd/` + `internal/intel/pd.go` so OSV can be removed later without touching CLI consumers.

Release builds embed gzip intel bundles for OSV and PD. CI runs `go run ./cmd/embedintel -out internal/bundle/embedded` before `go build`. Dev builds use committed minimal stubs from `make bundles-minimal` (bootstrap tag avoids chicken-and-egg with `//go:embed`).
...
|----------|---------|---------|
| `build-test.yml` | PR (Go changes) | golangci-lint + cross-OS build/test |
| `release-test.yml` | PR | GoReleaser snapshot build |
| `release-binary.yml` | `v*` tag push | GoReleaser release (full intel bundles + binaries) |
| `dep-auto-merge.yml` | Dependabot PRs | Auto-merge dependency updates |

Release requires repo secrets: `DEPX_PD_API_TOKEN` (full PD bundle), plus optional `RELEASE_SLACK_WEBHOOK`, `DISCORD_WEBHOOK_*`, `DEPENDABOT_PAT`.
