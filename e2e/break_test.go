...
func TestCLIBreakCases(t *testing.T) {
	...
	run := func(stdin string, args ...string) (stdout string, stderr string, exit int) {
		...
		cmd.Env = withoutEmbeddedSeedEnv(append(os.Environ(),
			"DEPX_OSV_URL="+osvURL,
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			case "oversized ref":
				if exit != 2 || !strings.Contains(stderr, "stdin line exceeds maximum size") {
					t.Fatalf("expected friendly oversized stdin error, exit=%d stderr=%q", exit, stderr)
				}
			case "npm empty name":
				if exit != 2 || !strings.Contains(stderr, "package name is required") {
					t.Fatalf("expected npm empty name usage error, exit=%d stderr=%q", exit, stderr)
				}
			}
		})
	}
