...
func TestE2ESyncOSVDelta(t *testing.T) {
	...
	// catalog is served by an httptest handler (separate goroutine) and mutated
	// by the test between runs, so guard it against the data race.
	var catalogMu sync.Mutex
	catalog := fmt.Sprintf("%s,npm/MAL-2026-TEST1\n", recent)
	setCatalog := func(s string) {
		catalogMu.Lock()
		defer catalogMu.Unlock()
		catalog = s
	}
	getCatalog := func() string {
		catalogMu.Lock()
		defer catalogMu.Unlock()
		return catalog
	}
	...
	setCatalog(fmt.Sprintf("%s,npm/MAL-2026-TEST1\n%s,npm/MAL-2026-TEST3\n", recent, recent))
