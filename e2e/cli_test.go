...
func TestCLI(t *testing.T) {
	...
	t.Run("version", func(t *testing.T) {
		...
		cmd.Env = append(os.Environ(), "DEPX_OSV_URL="+osvURL, "NO_COLOR=1")
	...
	t.Run("human feed no-color", func(t *testing.T) {
		...
		if !strings.Contains(out, "Showing") || !strings.Contains(out, "MAL-2026-TEST1") {
			t.Fatalf("expected human feed cards: %s", out)
		}
	})
