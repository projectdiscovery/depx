...
// TestE2EUXTimingLiveOptional runs against the real inventory when DEPX_UX_LIVE=1.
func TestE2EUXTimingLiveOptional(t *testing.T) {
	if os.Getenv("DEPX_UX_LIVE") != "1" {
		t.Skip("set DEPX_UX_LIVE=1 to benchmark the real inventory (slow on cold cache)")
	}
	if os.Getenv("DEPX_SOURCE_URL") != "" {
		t.Skip("unset DEPX_SOURCE_URL for live UX bench")
	}
