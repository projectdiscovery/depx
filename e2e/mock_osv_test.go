...
func registerOSVAPIRoutes(mux *http.ServeMux) {
	mux.HandleFunc("/v1/vulns/MAL-2026-TEST1", func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"id":"MAL-2026-TEST1","summary":"Malicious code in evil-pkg (npm)","references":[{"type":"WEB","url":"https://example.com/evil-pkg"}],"affected":[{"package":{"name":"evil-pkg","ecosystem":"npm"}}]}`))
	})
	mux.HandleFunc("/v1/vulns/GHSCAN-MAL-1", func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"id":"GHSCAN-MAL-1","summary":"Malicious code in evil-pkg (npm)","references":[{"type":"WEB","url":"https://example.com/evil-pkg"}],"affected":[{"package":{"name":"evil-pkg","ecosystem":"npm"}}]}`))
	})
	...
	mux.HandleFunc("/v1/vulns/MAL-2026-TEST2", func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"id":"MAL-2026-TEST2","summary":"Malicious code in bad-pkg (PyPI)","affected":[{"package":{"name":"bad-pkg","ecosystem":"PyPI"}}]}`))
	})
