...
func writeFeedDashboard(w io.Writer, c aurora.Aurora, result *feed.Result) {
	...
	if campaigns := filterCampaignBuckets(result.Window.Campaigns); len(campaigns) > 0 {
		writeSubHeader(w, c, "top campaigns", "")
		writeCountBars(w, c, campaigns)
		fmt.Fprintln(w)
	}

	if result.Window.Withdrawn > 0 || result.Window.WithAliases > 0 {
		writeSubHeader(w, c, "window status", "")
		writeKV(w, c, "withdrawn", fmt.Sprintf("%d", result.Window.Withdrawn))
...
func filterCampaignBuckets(buckets []source.CountBucket) []source.CountBucket {
	if len(buckets) == 0 {
		return nil
	}
	out := make([]source.CountBucket, 0, len(buckets))
	for _, b := range buckets {
		if strings.EqualFold(b.Label, "unknown") {
			continue
		}
		out = append(out, b)
	}
