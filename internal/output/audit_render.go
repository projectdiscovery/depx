...
func writeAuditResults(o Options, result *audit.Result) {
	...
	if result.Summary.Malicious == 0 && result.Summary.Quarantined == 0 {
...
func auditVerdictLabel(c aurora.Aurora, summary audit.Summary) string {
	...
	if summary.Malicious > 0 {
		parts = append(parts, c.Red(fmt.Sprintf("MALICIOUS (%d)", summary.Malicious)).String())
	}
	if summary.Quarantined > 0 {
		parts = append(parts, quarantineVerdictStyle(c, fmt.Sprintf("QUARANTINED (%d)", summary.Quarantined)))
	}
	if len(parts) > 0 {
...
func writeAuditFindingsFooter(w io.Writer, c aurora.Aurora, result *audit.Result) {
	...
	if result.FindingsStreamed {
		shown = result.Summary.Malicious + result.Summary.Quarantined
	}
	if shown == 0 {
		return
	}
	msg := fmt.Sprintf("↳ %d finding", shown)
	if shown != 1 {
		msg += "s"
	}
	if result.Summary.Malicious > 0 && result.Summary.Quarantined == 0 {
