...
func (s *Service) shouldSkipPlaceholderMatch(ctx context.Context, dep Dependency) bool {
...
func (s *Service) AuditWithOptions(ctx context.Context, paths []string, opts Options) (*Result, error) {
	...
	findings, maliciousCount, cleanCount, skippedPlaceholders, err := s.matchLocally(ctx, allDeps, opts)
	if err != nil {
		return nil, err
	}
...
func (s *Service) matchLocally(ctx context.Context, deps []Dependency, opts Options) ([]Finding, int, int, int, error) {
	...
	for i, dep := range deps {
		...
		if s.shouldSkipPlaceholderMatch(ctx, dep) {
			skippedPlaceholders++
			cleanCount++
			continue
		}
		...
		finding := enrichFinding(newFinding(dep, ids, summary, published, modified))
	...
	return findings, maliciousCount, cleanCount, skippedPlaceholders, nil
}
...
func newFinding(dep Dependency, ids []string, summary string, published, modified time.Time) Finding {
	f := Finding{
...
func (s *Service) fillFindingEnrichment(ctx context.Context, cache advisoryMetaCache, f *Finding) {
	...
	if f.Campaign != "" && !f.Published.IsZero() && !f.ModifiedAt.IsZero() {
		return
	}
	id := f.IDs[0]
	meta, ok := cache[id]
