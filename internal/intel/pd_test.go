...
func TestPDProviderFeedSortsByPublishedNotModified(t *testing.T) {
	modRecent := time.Now().UTC().Add(-1 * time.Hour).Format(time.RFC3339)
	pubRecent := time.Now().UTC().Add(-2 * time.Hour).Format(time.RFC3339)
	pubOlder := time.Now().UTC().Add(-48 * time.Hour).Format(time.RFC3339)
	modOlder := time.Now().UTC().Add(-3 * time.Hour).Format(time.RFC3339)
		...
		_ = json.NewEncoder(w).Encode(map[string]any{
			...
			"data": map[string]any{
				"packages": []map[string]any{
					{
						"mal_id": "MAL-MOD-NEW", "ecosystem": "npm", "pkg_name": "mod-first",
						"modified": modRecent, "published": pubOlder,
					},
					{
						"mal_id": "MAL-PUB-NEW", "ecosystem": "npm", "pkg_name": "pub-first",
						"modified": modOlder, "published": pubRecent,
					},
				},
			},
			"meta": map[string]any{"page": 1, "per_page": 100, "total": 2, "total_pages": 1},
		})
	}))
	defer srv.Close()
	...
	p, err := NewPD("test", &config.Config{Feed: config.FeedConfig{Since: 72 * time.Hour, Limit: 25}})
	if err != nil {
		t.Fatal(err)
	}
	...
	if resp.Entries[0].Name != "pub-first" {
		t.Fatalf("order = %q first, want pub-first (newest published)", resp.Entries[0].Name)
	}
}
