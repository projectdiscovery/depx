...
func (s *Service) Check(ctx context.Context, pkg ref.PackageRef) (*Result, error) {
	...
	q := malindex.QueryRequest{
		Package: &malindex.PackageQuery{
			Name:      pkg.Name,
	...
	// The intel source is the malicious-package inventory: every advisory it
	// returns for a package denotes a malicious entry, so presence in the index
	// (already version-matched by Query) is the verdict. We must not re-filter
	// by ID prefix here, or check would disagree with feed/search/id for any
	// entry whose primary ID is not MAL-/GHSCAN-MAL-.
	malicious := resp.Vulns
	result := &Result{
		Ref:              pkg.String(),
		PURL:             "pkg:" + pkg.Ecosystem + "/" + pkg.Name,
		Confidence:       "high",
	...
	case len(malicious) > 0:
		if s.isQuarantinedMatch(ctx, pkg.Ecosystem, pkg.Name, pkg.Version) {
			result.Verdict = VerdictQuarantined
			s.persistQuarantine(pkg.Ecosystem, pkg.Name)
		} else {
...
func (s *Service) CheckAllMany(ctx context.Context, items []BareRef) ([]Result, error) {
	...
	queries := make([]malindex.QueryRequest, 0, len(items)*len(ecosystems))
	...
	for i, item := range items {
		...
		if results[i].Verdict == VerdictMalicious {
			eco := quarantineEcosystem(results[i])
			if s.isQuarantinedMatch(ctx, eco, item.Name, item.Version) {
				results[i].Verdict = VerdictQuarantined
				s.persistQuarantine(eco, item.Name)
			}
		}
...
func applyQueryHits(result *Result, eco string, vulns []malindex.Vulnerability) {
	// Inventory hits are malicious by definition; see Check for rationale.
	malicious := vulns
	if len(malicious) == 0 {
		return
	}
...
func (s *Service) isQuarantinedMatch(ctx context.Context, ecosystem, name, version string) bool {
...
func quarantineEcosystem(result Result) string {
	if result.PackageEcosystem != "" {
		return result.PackageEcosystem
	}
	if len(result.MatchedEcosystems) == 1 {
		return result.MatchedEcosystems[0]
	}
	return "npm"
}
...
func (s *Service) buildAdvisoriesFromIDs(ctx context.Context, ids []string) []AdvisorySummary {
	...
	for _, id := range ids {
		out = append(out, s.advisorySummary(ctx, malindex.Vulnerability{ID: id}))
	}
	return out
}

func (s *Service) advisorySummary(ctx context.Context, stub malindex.Vulnerability) AdvisorySummary {
	v := stub
