...
func (e *Engine) LoadIndex(ctx context.Context, onProgress osv.IndexLoadProgress, onStatus osv.IndexLoadStatus) (*osv.MaliciousIndex, error) {
	...
	if blocking && !e.hasUsableCompiledIndex() {
		if err := e.waitForBackgroundSync(ctx, onProgress, onStatus); err != nil {
			return nil, err
		}
	}
	switch e.cfg.Source {
...
func (e *Engine) hasUsableCompiledIndex() bool {
	path := osv.CompiledCachePath(e.cfg.CacheDir, compiledName(e.cfg.Source))
	if idx, ok := osv.LoadCompiledIndexStale(path); ok && idx.PackageCount() > 0 {
		return true
	}
	return false
}

func (e *Engine) waitForBackgroundSync(ctx context.Context, onProgress osv.IndexLoadProgress, onStatus osv.IndexLoadStatus) error {
	e.mu.Lock()
	if !e.running {
		e.mu.Unlock()
		return nil
	...
	report := func() {
		if onStatus == nil && onProgress == nil {
			return
		}
		loaded, total := indexSyncProgress(e.cfg.CacheDir)
		if onProgress != nil {
			onProgress(loaded, total)
		}
		if onStatus != nil && loaded == 0 && total == 0 {
			onStatus(osv.FormatIndexDownloadProgress(0, 0))
		}
	}
	...
	for {
		...
		e.mu.Unlock()
		if !running {
			if onStatus != nil && e.hasUsableCompiledIndex() {
				path := osv.CompiledCachePath(e.cfg.CacheDir, compiledName(e.cfg.Source))
...
func countVulnBlobs(cacheDir string) int {
	entries, err := os.ReadDir(filepath.Join(cacheDir, osvBlobRel))
	if err != nil {
		return 0
	}
	n := 0
