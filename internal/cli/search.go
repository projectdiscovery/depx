...
func runSearch(cmd *cobra.Command, args []string) error {
	...
	if intelProvider != nil {
		...
		if firstRun {
			initialMsg = malindex.WithFirstRunCacheNote("Downloading malicious package inventory…")
		}
	}
	...
	result, err := svc.Run(cmd.Context(), search.Options{
		...
		OnStatus: func(msg string) {
			...
			if strings.HasPrefix(msg, "Downloading malicious package inventory") &&
				!strings.Contains(msg, malindex.FirstRunCacheNote) {
				msg = malindex.WithFirstRunCacheNote(msg)
			}
			sp.SetMessage(msg)
		},
		OnProgress: func(loaded, total int) {
			...
			msg := malindex.FormatIndexDownloadProgress(loaded, total)
			msg = malindex.WithFirstRunCacheNote(msg)
			sp.SetMessage(msg)
		},
	})
	sp.Stop()
	if err != nil {
		if os.Getenv("DEPX_SOURCE_URL") != "" {
			depxLog("DEPX_SOURCE_URL is set — unset it outside e2e tests")
			depxLog("Run: unset DEPX_SOURCE_URL")
		}
		return err
	}
	o := outOpts()
...
func maybeWarnSearchIndexIncomplete() {
	if os.Getenv("DEPX_SOURCE_URL") != "" {
		depxLog("DEPX_SOURCE_URL is set — index sync is disabled outside e2e tests")
		depxLog("Run: unset DEPX_SOURCE_URL")
		return
	}
	if intelProvider == nil {
	...
	if st.LastSuccess.IsZero() {
		...
		depxLog("First search downloads the malicious-package inventory once and caches it locally")
		if st.LastError != "" {
