...
func (c *Client) listRepos(ctx context.Context, endpoint, listOwner string, limit int, baseQuery url.Values) ([]Repo, error) {
	...
	for len(out) < limit {
		...
		// Always request full pages. Sizing per_page to the remaining count
		// (limit-len(out)) breaks pagination because archived repos are filtered
		// below: out grows slower than rows consumed, per_page shrinks, and the
		// short page then trips the end-of-results check prematurely. A full page
		// is still a single request, and out is capped at limit as we append.
		q.Set("per_page", fmt.Sprintf("%d", defaultRepoListPage))
