...
func (e *CodedError) Error() string {
	if e.Message != "" {
		// Append the wrapped error only when it carries detail. Category
		// sentinels (e.g. ErrUsage) are for classification/Unwrap, not display,
		// so we don't leak "...: usage error" into user-facing messages.
		if e.Err != nil && !isCategorySentinel(e.Err) {
...
func isCategorySentinel(err error) bool {
	switch err {
	case ErrUsage, ErrUpstream, ErrFindings, ErrNotFound, ErrNotConfig:
		return true
	default:
		return false
	}
