...
// FilterMALEntries returns malicious-package rows from a modified-index catalog.
func FilterMALEntries(entries []IndexEntry) []IndexEntry {
	return filterMALEntries(entries)
}

// BuildMaliciousIndexFromGCS downloads OSV ecosystem zip bundles (when large
// enough) and individual GCS records otherwise, writing vuln blobs to cacheDir.
func BuildMaliciousIndexFromGCS(ctx context.Context, cacheDir, userAgent string, timeout time.Duration, entries []IndexEntry, onProgress IndexLoadProgress, onStatus IndexLoadStatus) (*MaliciousIndex, error) {
	client := NewClient("bulk", timeout, WithCache(cacheDir, 0), WithUserAgent(userAgent))
	return client.buildMALIndexFromGCS(ctx, filterMALEntries(entries), onProgress, onStatus)
}
...
// LookupByID returns package metadata for a malicious advisory ID in the compiled index.
